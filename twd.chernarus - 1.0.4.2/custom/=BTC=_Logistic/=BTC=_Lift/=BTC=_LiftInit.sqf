...
while {true} do 
{
	...
	if (!(vehicle player == player) && driver vehicle player == player) then {_chopper = vehicle player;BTC_liftHudId = _chopper addAction [("<t color=""#ED2744"">" + ("Hud On\Off") + "</t>"),"custom\=BTC=_Logistic\=BTC=_Lift\=BTC=_Hud.sqf", "", 0, false, false];};
	...
	while {!(vehicle player == player) && driver vehicle player == player} do
	{
		...
		if (!isNull _cargo && BTC_lifted == 0 && _can_lift && format ["%1", _cargo getVariable "BTC_Cannot_Lift"] != "1") then 
		{
			if (((abs _cargo_z) < BTC_lift_max_h) && ((abs _cargo_z) > BTC_lift_min_h) && ((abs _cargo_x) < BTC_lift_radius) && ((abs _cargo_y) < BTC_lift_radius)) then
			{
				...
				{
					BTC_liftActionId = _chopper addAction [_text_action,"custom\=BTC=_Logistic\=BTC=_Lift\=BTC=_attachCargo.sqf", "", 7, true, true];
					BTC_lift = 0;
				};
			};
