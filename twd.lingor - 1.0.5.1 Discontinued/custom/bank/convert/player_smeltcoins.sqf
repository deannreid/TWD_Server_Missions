...
if (_max_bars < 1) exitWith {
	...
	cutText [format["You need another %1 %2 to smelt a %3.",_needed,CurrencyName,_type] , "PLAIN DOWN"];
	SmeltingInProgress = false;
};
...
for "_x" from 1 to _max_bars do {
	...
	if (_finished) then {
		...
		if (_player_wealth >= _rate) then {
			player setVariable["cashMoney",(_player_wealth - _rate),true];
			player addMagazine _type;
			_abort = false;
			cutText [format["Smelted %1 %2 into a %3",_rate,CurrencyName,_type], "PLAIN DOWN"];
		} else {
			_needed =  _rate - _player_wealth;
			cutText [format["You need another %1 %2 to smelt a %3.",_needed,CurrencyName,_type] , "PLAIN DOWN"];
			_abort = true;
