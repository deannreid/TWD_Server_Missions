...
if (!isServer) exitWith {diag_log "server_deleteSafeZoneVehicles.sqf - ERROR: NOT SERVER?"};
...
if(isNil "HALV_VEHICLE_CLEANUPZONES")then{
	...
	switch (_WorldName)do {
		case "chernarus":{
			diag_log format["%2: Cleanup zones for chernarus selected! (check: %1)",_WorldName,_txt];
			HALV_VEHICLE_CLEANUPZONES = [
			//position				radius	cityname
			...
			[[4486.37,10313.5,0	],	100,		"Aircraft Dealer NE"],
			[[12429.5,12564,0		], 	100, 	"Aircraft Dealer NW"],
		...
		case "tavi":{
			...
			[[9859.4209,7471.5684,0],	75,			"Wholesaler"]
			];
		};
		default{
