...
player removeAction s_player_doorManagement;
s_player_doorManagement = -1;

//Allow manage door
if((_typeOfCursorTarget in DZE_DoorsLocked)) then {
	if (s_player_manageDoor < 0) then {
		s_player_manageDoor = player addAction ["<t color='#0059FF'>Manage Door</t>", "custom\doorManagement\initDoorManagement.sqf",[_cursorTarget], 0, false, true, "",""];
	};
} else {
