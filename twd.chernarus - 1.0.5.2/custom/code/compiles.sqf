...
//Custom Compiles
	if (_debugMonitor) then{diag_log "Initialising VL Compiles";};
...
//Custom Zeds
	if (_debugMonitor) then{diag_log "Initialising Custom Zeds";};
	...
	if (_debugMonitor) then{diag_log "Finished initialising Custom Zeds";};	

	
	
	
//Custom Loot	
	if (_debugMonitor) then{diag_log "Initialising Custom Loot";};
	...
	if (_debugMonitor) then{diag_log "Finished initialising Custom Loot";};
	
	
	
	
//GUI
	if (_debugMonitor) then{diag_log "Injecting GUI";};
	player_updateGui =			compile preprocessFileLineNumbers "custom\GUI\player_updateGui.sqf";
	if (_debugMonitor) then{diag_log "GUI successfully Injected";};
	
	
	
	
//Anims
	if (_debugMonitor) then{diag_log "Loading Animations";};
		if (!isDedicated) then {mv22_pack = compile preprocessFileLineNumbers "\ca\air2\mv22\scripts\pack.sqf";};
	if (_debugMonitor) then{diag_log "Finished Loading Animations";};




//Temp Fixes
	if (_debugMonitor) then{diag_log "Loading Temp Fixes to Epoch";};
//		player_craftItem =			compile preprocessFileLineNumbers "temp\player_craftItem.sqf";
	if (_debugMonitor) then{diag_log "No Fixes Defined";};	




	if (_debugMonitor) then{diag_log "Loading Group Management, Plot Management and Door Management";};
if (!isDedicated) then {
	...
	if (_debugMonitor) then{diag_log "Finished loading Group Management, Plot Management and Door Management";};

	if (_debugMonitor) then{diag_log "Loading Microsoft Paint";};
	...
	if (_debugMonitor) then{diag_log "Finished Loading Microsoft Paint";};


	if (_debugMonitor) then{diag_log "Finished Initialising VL Compiles";};
