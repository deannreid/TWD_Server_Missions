if(isServer) then {
	...
	dynamic_crate 					= compile preprocessFileLineNumbers "\z\addons\twd_server\WAI\compile\dynamic_crate.sqf";
	custom_publish  				= compile preprocessFileLineNumbers "\z\addons\twd_server\WAI\compile\custom_publish_vehicle.sqf";

	// Mission functions
	call 							compile preprocessFileLineNumbers "\z\addons\twd_server\WAI\compile\mission_functions.sqf";
	mission_init					= compile preprocessFileLineNumbers "\z\addons\twd_server\WAI\compile\mission_init.sqf";
	mission_winorfail				= compile preprocessFileLineNumbers "\z\addons\twd_server\WAI\compile\mission_winorfail.sqf";
	minefield						= compile preprocessFileLineNumbers "\z\addons\twd_server\WAI\compile\minefield.sqf";
	...
	while {true} do
	{
		...
		if((_cnt >= wai_players_online) && (markerready) && ((diag_fps) >= wai_server_fps)) then {
			...
			if (_result == 1) then {
				...
				execVM format ["\z\addons\twd_server\WAI\missions\hero\%1.sqf",_mission];
			};
			...
			if (_result == 2) then {
				...
				execVM format ["\z\addons\twd_server\WAI\missions\bandit\%1.sqf",_mission];
			};
			...
			if (_result == 3) then {
				...
				execVM format ["\z\addons\twd_server\WAI\missions\special\%1.sqf",_mission];
			};
