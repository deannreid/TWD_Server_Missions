...
if (_button == 1) then {
	...
	if (isClass _erc_cfgActions) then {
		...
		// Add extra context menus 2
	_erc2_cfgActions = (missionConfigFile >> "ExtraRc2" >> _item);
	_erc2_numActions = (count _erc2_cfgActions);
	if (isClass _erc2_cfgActions) then {
    for "_j" from 0 to (_erc2_numActions - 1) do 
    {
        _menu =  _parent displayCtrl (1600 + _j + _numActions);
        _menu ctrlShow true;
        _config =  (_erc2_cfgActions select _j);
        _text =  getText (_config >> "text");
        _script =  getText (_config >> "script");
        _height = _height + (0.025 * safezoneH);
        uiNamespace setVariable ['uiControl', _control];
