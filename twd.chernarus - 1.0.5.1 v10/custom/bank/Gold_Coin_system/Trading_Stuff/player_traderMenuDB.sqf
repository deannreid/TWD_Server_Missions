...
TraderDialogLoadItemList = {
	...
	_item_list = [];
	{
		...
		_order = _x select 5; //
		_afile = _x select 7;
		_File = "custom\bank\Gold_Coin_system\Trading_Stuff\" + _afile + ".sqf";
		_count = 0;
...
TraderDialogBuy = {
	...
	if(_item select 9 == "custom\bank\Gold_Coin_system\Trading_Stuff\trade_items.sqf")then{

	_data = [_item select 0, _item select 1, _item select 2, _item select 8];
	_data execVM "custom\bank\Gold_Coin_system\Trading_Stuff\trade_items_buy.sqf";
...
TraderDialogSell = {
	...
	_data execVM "custom\bank\Gold_Coin_system\Trading_Stuff\trade_items_sell.sqf";
